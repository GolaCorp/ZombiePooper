...
export class HomePage {
    ...
    ionViewWillEnter() {
        ...
        for (var i = 10; i < 20; i++) {
            this.levels.push({
                ...
                fire_force: i < 13 ? 1.3 : (i < 17 ? 1.66 : 2),
                ghost_speed: 1.5 + i * 0.4 * Math.random(),
                fire_speed: 2 + i * 0.1 - 0.5,
                shooter_level: i < 20 ? 3 : (i < 5 ? 4 : (i < 8 ? 5 : 6)),
    ...
    fire(level) {
        for (var i = 0; i < level; i++) {
            ...
            var vel = this.Vector.sub(this.Vector.create(this.shooter.vertices[i].x, this.shooter.vertices[i].y), this.Vector.create(this.shooter.position.x, this.shooter.position.y));
    ...
    checkIsIn() {
        ...
        if (this.score >= this.levels[this.currentlevel].target_score) {
            ...
            this.navCtrl.push(GameoverPage, {
                image: this.render.canvas.toDataURL("image/jpeg")
                , score: this.score, width: this.width, height: this.height, next: true
            }).then(() => {
    ...
    init() {
        ...
        this.mouse = this.Mouse.create(this.gameCanvas.nativeElement);

        this.world.gravity.y = 0;

        this.shooter = this.Bodies.polygon(this.width / 2, this.height / 2, this.shooter_level, 24, {
            collisionFilter: {
                mask: this.noCollisions | this.collisions | this.dragMask
            },
            render: {
                fillStyle: 'white',
                strokeStyle: 'white'
        ...
        this.mConstraint = this.MouseConstraint.create(
            this.engine,{ 
                mouse: this.mouse,
                stiffness: 0.2,
                collisionFilter: {
        ...
        this.Events.on(this.engine, 'collisionStart', (event) => {
            ...
            for (var i = 0; i < pairs.length; i++) {
                ...
                if (pair.bodyA.label == "Polygon Body" || pair.bodyB.label == "Polygon Body") {
                    ...
                    this.navCtrl.push(GameoverPage, {
                        image: this.render.canvas.toDataURL("image/jpeg")
                        , score: this.score, width: this.width, height: this.height, next: false
                    }).then(() => {
                        this.navCtrl.remove(currentIndex);
                    });
                }
    ...
    keyTouchMouse() {
        document.addEventListener('touchmove', (p) => {
            var pos = p.touches[0];
            // this.Body.translate(this.shooter, {
            //     x: pos.screenX,
            //     y: pos.screenY
            // });
            // var dir = this.Vector.create((pos.screenX - this.touchinit.screenX), (pos.screenY - this.touchinit.screenY));
            // dir = this.Vector.normalise(dir);
            // dir = this.Vector.mult(dir, 2);
            // this.Body.translate(this.shooter, {x: this.shooter.position.x+ dir.x, y: this.shooter.position.y+dir.y})
